...
create_model_paths_yaml() {
    if [ -n "$MODEL_BASE_PATH" ]; then
        # Check if MODEL_BASE_PATH exists and is accessible
...
while [ $restart_count -lt $MAX_RESTART_COUNT ]; do
    ...
    if [ $server_ready -eq 0 ]; then
        ...
        while true; do
            ...
            # Check if server is still responding (with shorter timeout for monitoring)
            if ! check_server $SERVER_PORT 3; then
